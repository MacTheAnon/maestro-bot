...
=========================================================
intents = discord.Intents.default() intents.message_content = True intents.members = True client = discord.Client(intents=intents)

@client.event async def on_ready(): print(f'⚡ {client.user} is online! Type "@MaestroBot hello" to test.')

@client.event async def on_member_join(member): role_name = "FebruaryCohort" role = discord.utils.get(member.guild.roles, name=role_name) if role: try: await member.add_roles(role) print(f"✅ Assigned {role_name} to {member.name}") except Exception as e: print(f"❌ Failed to assign role: {e}") else: print(f"⚠️ Role '{role_name}' not found in this server.")
...
if name == "main": client.run(DISCORD_TOKEN)
